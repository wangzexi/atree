...
import { findSessionStore, readSessionStore, readWorkspaceSessionStoresDeep } from "./session-store"
...
export const resolveFileSession = Effect.fn("Atree.resolveFileSession")(function* (input: {
  ...
  const instanceDirectory = input.instanceDirectory
  if (instanceDirectory && !sameDirectory(instanceDirectory, input.directory)) {
    const found = yield* Effect.promise(() => findSessionStore(path.resolve(instanceDirectory), input.sessionID)).pipe(
      Effect.catchCause(() => Effect.succeed(undefined)),
    )
    if (found) return found
  }
  ...
  let found: FileSession | undefined
  for (const session of sessions) {
    ...
    if (found.directory !== session.directory) return
  }
  if (found) return found
})
