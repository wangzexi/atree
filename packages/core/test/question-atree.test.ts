...
describe("QuestionV2 atree state", () => {
  ...
  it.effect("does not append question events into another copied session when no explicit directory is given", () =>
    Effect.gen(function* () {
      const data = yield* Effect.acquireRelease(
        Effect.promise(() => mkdtemp(path.join(os.tmpdir(), "atree-core-question-data-"))),
        (dir) => Effect.promise(() => rm(dir, { recursive: true, force: true })).pipe(Effect.ignore),
      ...
      const sessionID = SessionV2.ID.make("ses_core_question_no_explicit_directory")
      ...
      const fiber = yield* service
        .ask({
          ...
          questions: [
            {
              question: "Should not leak into another directory without an explicit hint?",
              header: "Implicit",
              options: [{ label: "No", description: "Keep it local only" }],
            },
          ],
        })
        .pipe(Effect.forkScoped)
