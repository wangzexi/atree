...
会话保存为事件流。具体 schema 后续参考 Pi Coding Agent / pi-mono 的实现，但设计原则先固定：
...
- 需要支持会话流式事件。
- 会话执行优先参考 Pi Coding Agent 的 TypeScript SDK；如果以后需要从 Python 接入，可参考 Pi 的 RPC 模式，而不是假设存在独立 Python SDK。
...
- Pi Coding Agent：JSONL 会话、`AgentSession`、事件订阅、图片输入、RPC 模式。
- pi-mono web-ui：可参考 ChatPanel/AgentInterface 的交互和事件模型，但不要照搬 IndexedDB 存储。
- opencode：可参考本地 HTTP GUI 启动、受保护 API、SSE 事件流、前端批量消费事件。
