...
1. ~~**补 JSONL 链路集成测试**~~ ✓ 已完成
2. ~~**清理 V1 遗留表双写**~~ ✓ 已完成
3. ~~**server 包 SessionLocationMiddleware 深度搜索**~~ ✓ 已完成
4. ~~**修复 getWithDirectory 目录边界**~~ ✓ 已完成（嵌套子目录 session 不再从父目录访问）
5. ~~**修复 admittedSeq 幂等性**~~ ✓ 已完成（prompt 重试返回相同 admittedSeq）
6. ~~**修复 workspace 路由目录**~~ ✓ 已完成（本地 workspace 用 session 实际目录而非 workspace 数据目录）
7. ~~**修复全部过时测试断言**~~ ✓ 已完成（cursor、corrupt 消息、summary diffs、schema 等）
8. **Playwright 冒烟测试**：覆盖 选目录→开会话→发消息→设自动化→触发→header 消失 的 UI 主流程
9. **projector 运行时解耦**：将 `SessionMessageUpdater` 从 `SessionMessageTable` 迁移到 JSONL 重放（执行路径，非加载路径）
10. **收窄 HTTP 接口**：清理剩余 OpenCode 原始接口
