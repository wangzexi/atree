...
这个分支作为 atree 主分支，历史设计文档统一放在 `docs/` 下，按用途如下：
...
本次分支晋级约定：
当前分支（`main`）作为 atree 主线。若原主线有未迁移内容，统一放到旧主分支备份中，不再回写到新主线。

本分支已定为 atree 主线分支（`main`）。历史主分支快照已保留为：
...
- `main-legacy-before-atree-ng-mainline-finalized`（当前主干接管时的当前主干镜像备份）
