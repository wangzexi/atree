...
### 1. 目录内事实源优先，不保留业务 fallback

atree 不应该把会话、自动化和执行历史的唯一事实源放进全局数据库。

第二版的目标不是“双写后再兜底读取全局库”，而是把目录内 `.agents/atree/` 明确为唯一业务事实源。

也就是说：

- 目录内存在会话事实源时，不再回退到全局 SQLite 继续补读消息、自动化或元数据。
- 显式按目录访问会话时，如果该目录下没有对应会话，就应该直接视为不存在。
- 同一个 session id 在多个复制目录中出现时，也不应该靠全局库去“猜”哪个才是真的。
- 所有“找不到目录事实源就从全局库兜底”的逻辑，都应逐步删除，而不是长期保留。
...
如果某项数据丢失后会改变某个目录里的业务语义，那么它就不该只存全局层。
...
这里的过渡只允许发生在“同一会话目录内部”，不允许再向全局数据库要一层业务 fallback。
...
```text
目录里的 .agents/atree = 真相
SQLite = 可重建投影
```

## 第二版实施立场

这一版不再把“平滑兼容 OpenCode 全局存储”作为目标。

更直接地说：

- 不做业务 fallback。
- 不做“目录读不到就去 SQLite 补”。
- 不做“为了兼容旧接口，继续让全局库决定目录语义”。

允许的妥协只有一种：

- 某些运行表暂时保留，但它们必须是可丢弃、可重建、不可作为业务真相的。
...
那它就不是运行表，而仍然是业务事实源。这类数据必须继续往 `.agents/atree/` 迁。

## 对当前 spike 的直接要求

基于这个立场，当前 OpenCode spike 后续要按下面的顺序推进：

1. 先用测试固定现有目录自包含行为。
2. 再删除剩余“目录不存在时回落到全局库”的业务读取。
3. 然后把 schedule/todo/question/permission/context 等读写入口继续向 `session.jsonl` 和会话目录收敛。
4. 最后再决定是继续瘦身 OpenCode backend，还是直接切 Pi core。

顺序不能反。否则接口层改薄了，但底层事实源还是双轨，后面还得返工。

如果投影脏了，应该重建；不应该反过来覆盖目录真相。
...
- 复制 `.agents/atree/` 到另一个目录后，`session.jsonl` 中的 `assets/...` 相对路径会从新的包含目录解析；会话资产不会继续指向源目录或依赖全局缓存。
...
- core `SessionTodo.update` 在找到 file-backed session 但没有 SQLite session row 时，会只写当前目录的 `todo.json` 和 `session.jsonl`，不再强行写 `TodoTable` 导致目录-only 会话被全局 DB 外键卡住。
- core `SessionContextEpoch.prepare` 在只有 file-backed session、没有 SQLite session row 时，会从目录会话恢复一个可重建的运行时投影，再继续原有 epoch 流程；系统上下文更新仍会写回当前目录的 `session.jsonl`。
...
- core `SessionV2.messages/message/context` 和 `SessionStore.context/runnerEntries` 在定位到 file-backed session 后，即使 `session.jsonl` 还没有消息，也会把目录空日志视为事实源返回空结果，不再读取旧 SQLite message/context 投影。
...
- opencode 侧 `session.jsonl` 投影也开始恢复 durable shell 和 compaction 事件：shell started/ended 会投影成现有 V1 tool part，compaction started/ended 会投影成 compaction marker 和 summary assistant，确保这两类会话事实不再只依赖全局 EventV2/SQLite 投影。
...
- todo 的显式目录读取如果找不到该目录下的 file-backed session，会返回空列表，不再读取全局 SQLite `TodoTable` 中的旧投影；当已经定位到 file-backed session 时，即使目录里还没有 `todo.json` 或 `todo.updated` 事件，也以目录缺失状态为准返回空列表，避免旧 DB todo 被复活。
...
- 复制 `.agents/atree/` 到另一个目录后，question/permission 的 pending 扫描和服务恢复都会按“包含目录 + 会话 + request id”隔离；同一个 request id 出现在源目录和目标目录时不会被互相覆盖，且目标目录中的 replied/rejected 只清理目标目录自己的 pending。恢复出的 pending 被回复时，reply/reject 事件也会写回该 pending 所在的会话目录，而不是按同 session id 回落到 root 下第一个匹配目录。
- 这一点和 `SessionInputTable` / `SessionContextEpochTable` 不同：后两者当前还是按 `session_id` 单键表达的运行锁/运行队列，适合“同一会话一个全局运行态”的前提，不适合“source/target 两个目录副本同时保有独立运行态”的最终模型。第二版如果要把 prompt 队列、context replacement 也彻底目录化，需要先调整它们的 key 设计，而不是继续在现有全局表上追加目录猜测逻辑。
...
- workspace sync history replay 到本地的 `session.*` 事件会同步镜像到对应目录会话的 `session.jsonl`，并刷新 `meta.yaml` 投影；远端 workspace 历史不会只更新全局 EventV2/SQLite 投影而绕过目录事实源。
...
- `EventV2` 的 durable event log 还没有完整迁移到每个目录的 `session.jsonl`；当前 core reader 已能恢复 prompted 用户消息和 assistant step/text/reasoning/tool，`session.next.*` durable 边界事件、question/permission 事件已开始写入 JSONL，并且 pending 列表可以从 JSONL 重放恢复；但恢复后的 pending 如何重新绑定原本等待中的执行 fiber 仍待单独设计。
...
- worktree 等派生产物仍会落到全局 data dir；snapshot runtime 已迁入目录内，但其它没有明确 session/location 上下文的产物仍需要后续单独设计迁移路径。
