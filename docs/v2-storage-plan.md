...
- prompt 用户消息、自动标题、主循环 assistant 初始化/收尾、subtask assistant/tool 写入、summary/revert 清理、processor tool-call 元数据写入开始显式携带 session 目录上下文，减少对外层 InstanceState 和全局 SQLite row 的隐性依赖。
