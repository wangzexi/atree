...
- core 的 `SessionStore` 对“无显式目录”的解析也继续收紧到当前 `Location` 作用域：只要调用链已经带着当前目录，它的 `list/get/context/runnerContext/message` 就会优先读取该目录树下的 file-backed session / message，不再把 persisted root 下的 copied sibling 会话混进当前目录结果里。
...
- core `SessionV2.prompt` 在 persisted root 下遇到 copied 同 id 目录、且没有显式目录 hint 时，也已经有护栏保证“不猜目录、不写 prompt state、不补 `SessionInputTable`”；这让 prompt 生命周期和 question/permission 一样，继续服从目录歧义即失败的规则。
...
- 同一条规则现在也覆盖到了已有 runtime row 的保活路径：如果 `ScheduleTable` 里已经有一条运行投影，但 persisted root 下出现了多个同 id copied session、且当前没有显式目录 hint，`ensureScheduleStillExists()` 会把它视为 `ambiguous` 而不是 `missing`；`tick()`、`process()` 和启动 hydration 都只会跳过，不会误删这条 runtime row，更不会顺手停掉另一个目录副本仍然有效的自动化消息。
...
- `packages/opencode/src/session/session.ts` 里的 file-backed cache 补建路径也已经对齐：`ensureFileSessionCache()` 即使在缺行重建时也只写最小运行投影，而目录 patch 主链路本身继续保持“只写目录、不顺手重建 SQLite 行”。
...
- opencode `Session` 的目录元数据 patch 也继续收紧：
  ...
  - `message/part updated/removed` 现在也不再从 `Session.appendSessionEvent(...)` 主动补 `SessionTable` cache row。
