...
- `SessionV2.messages/context/message` 已能读取目录内 `session.jsonl` 的用户/助手文本、reasoning、文件资产和已完成工具调用结果投影。
...
- core `SessionV2` 已经能恢复文本、reasoning、文件资产和已完成工具调用结果，但还没有覆盖所有运行态事件，例如 pending/running tool、shell、compaction、permission/question 等更完整的 v2 结构。
