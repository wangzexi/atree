...
- `SessionV2.messages/context/message` 已能读取目录内 `session.jsonl` 的用户/助手文本、reasoning、文件资产、event-backed assistant step/text/reasoning、agent/model/context/synthetic 直接事件、pending/running/completed 工具调用、shell 事件和 compaction 事件投影。
...
- core `SessionV2` 已经能恢复文本、reasoning、文件资产、event-backed assistant step/text/reasoning、agent/model/context/synthetic 直接事件、pending/running/completed 工具调用、shell 事件和 compaction 事件，但还没有覆盖 event-backed tool、permission/question 等更完整的 v2 结构。
